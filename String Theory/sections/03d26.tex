...
Let's first consider the ground state, $\ket{0;p}$. Due to our normal ordering $N=\widetilde{N}=0$. This is a unique state (it's given by \Cref{eqn:VacuumState} and \Cref{eqn:GroundStateP}), and so it is clearly Lorentz invariant (there is nothing it can be rotated into).
